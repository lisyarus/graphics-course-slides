...
\pause
\item Если атрибут объявлен в шейдере как floating-point (\mintinline{glsl}|vec3| и т.п.), нужно использовать функцию \mintinline{cpp}|glVertexAttribPointer|, но храниться он \textbf{\textit{может}} как целочисленный
\pause
\item Если атрибут объявлен в шейдере как целочисленный (\mintinline{glsl}|ivec3,uvec4| и т.п.), нужно использовать функцию \mintinline{cpp}|glVertexAttribIPointer|, и храниться он \textbf{\textit{должен}} как целочисленный
\end{itemize}
\end{frame}
