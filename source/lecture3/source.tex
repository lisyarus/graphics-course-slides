...
\pause
\item Vertex Array -- описывают формат и хранение вершин
\pause
\item Текстуры -- изображения в памяти GPU, которые можно читать из шейдера, и в которые можно рисовать
\pause
\item Renderbuffer -- буферы, в которые можно рисовать
\pause
\item Framebuffer -- содержит настройки рисования в текстуры и renderbuffer'ы
\pause
\item И другие
\end{itemize}
\end{frame}
...
\begin{itemize}
\item В коде объект представляется идентификатором типа \verb|GLuint|
\begin{itemize}
...
\item Исключения:
\begin{itemize}
\item Framebuffer с нулевым id -- default framebuffer, рисует в окно, привязанное к контексту OpenGL, его нельзя настраивать
\pause
\item \textit{В OpenGL ES}: vertex array с нулевым id -- ничем не отличается от других vertex array'ев, но существует (создан) по умолчанию
\end{itemize}
\pause
\item Функции создания объектов никогда не возвращают нулевой id
\item Функции удаления объектов игнорируют нулевой id (т.е. удалить нулевой объект -- не ошибка)
\pause
...
\end{itemize}
\pause
\item Функции рисования (\verb|glDrawArrays| и др.) используют текущую шейдерную программу и текущий vertex array
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Для каждого вида объектов (буферы, текстуры, ...) есть отдельный набор возможных значений target
\pause
\item Для буферов, текстур, framebuffer'ов и renderbuffer'ов нет одного текущего объекта, но есть текущий объект для каждого конкретного target'а
\pause
\item Можно считать, что есть словарь \begin{math}\operatorname{target} \rightarrow \operatorname{id}\end{math} текущих объектов
\pause
...
\pause
\item \verb|GL_ELEMENT_ARRAY_BUFFER| (EBO/IBO) -- массив индексов вершин
\pause
\item \verb|GL_UNIFORM_BUFFER| (UBO) -- массив для uniform-блоков
\pause
\item ...и другие
\end{itemize}
\pause
\item Текущий \verb|GL_ELEMENT_ARRAY_BUFFER| хранится не глобально, а {\color{red}в текущем VAO!}
\end{itemize}
\end{frame}
...
\pause
\begin{itemize}
\item \begin{math}\Rightarrow\end{math} Драйвер, возможно, сначала копирует данные в собственную память
\end{itemize}
\end{itemize}
\end{frame}
...
\pause
\item \verb|glMapBuffer(target, access)| -- возвращает особый \textit{mapped} указатель
...
\pause
\item \verb|glUnmapBuffer| может вернуть \verb|GL_FALSE|, если что-то пошло не так, и память буфера при этом остаётся непроинициализированной
\end{itemize}
\end{frame}
...
\pause
\item Все вершины в рамках одного вызова команды рисования (\verb|glDrawArrays| и др.) имеет один набор атрибутов
\pause
...
\frametitle{Атрибуты вершин}
\begin{itemize}
\item Индекс: \verb|0, 1, ... glGet(GL_MAX_VERTEX_ATTRIBS) - 1|
\begin{itemize}
...
\item Состояние хранится в vertex array
\end{itemize}
\pause
...
\begin{itemize}
\item \verb|float: 0.0|
\item \verb|vec2: (0.0, 0.0)|
\item \verb|vec3: (0.0, 0.0, 0.0)|
\item \verb|vec4: (0.0, 0.0, 0.0, 1.0)|
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Как интерпретировать значение атрибута, который в шейдере объявлен как floating-point (\verb|float|, \verb|vec3|, и т.п.), но в данных он хранится как целочисленный?
\pause
\item 2 варианта:
\begin{itemize}
...
\pause
\item Полезно для компактного хранения атрибутов
\pause
...
\pause
\item Если атрибут объявлен в шейдере как floating-point (\verb|vec3| и т.п.), нужно использовать функцию \verb|glVertexAttribPointer|, но храниться он \textit{может} как целочисленный
\item Если атрибут объявлен в шейдере как целочисленный (\verb|ivec3,uvec4| и т.п.), нужно использовать функцию \verb|glVertexAttribIPointer|, и храниться он \textit{должен} как целочисленный
\end{itemize}
\end{frame}
...
\pause
\begin{itemize}
\item \verb|index| -- индекс аттрибута (\verb|layout (location = index)| в шейдере)
\pause
\item \verb|size| -- размерность (число компонент)
\pause
\item \verb|type| -- тип каждой компоненты
\pause
\item \verb|normalized| -- нормированный или нет
\pause
\item \verb|stride| -- расстояние между соседними значениями (в байтах)
\pause
\item \verb|pointer| -- указатель на данные
\pause
\begin{itemize}
\item На самом деле, \verb|pointer| -- сдвиг в байтах относительно начала памяти текущего \verb|GL_ARRAY_BUFFER|
\item Например, если данные этого атрибута начинаются на 12ом байте текущего \verb|GL_ARRAY_BUFFER|, нужно передать \verb|(void *)(12)|
\pause
...
\begin{itemize}
\item Размерность атрибута, указанная в \verb|glVertexAttribPointer| (т.е. реальное количество хранимых в памяти компонент), не обязана совпадать с размерностью, указанной в шейдере
\pause
...
\item id текущего \verb|GL_ELEMENT_ARRAY_BUFFER| хранится {\color{red}в текущем VAO!}
\pause
\begin{verbatim}
