...
\pause
\begin{itemize}
\item \alert{\textbf{Зачет}}: \textbf{50 и более} баллов
\pause
\item \alert{\textbf{Экзамен}}:
\begin{itemize}
\item \textbf{50-59 баллов}: \alert{\textbf{E}}
\item \textbf{60-69 баллов}: \alert{\textbf{D}}
\item \textbf{70-79 баллов}: \alert{\textbf{C}}
\item \textbf{80-89 баллов}: \alert{\textbf{B}}
\item \textbf{90-100 баллов}: \alert{\textbf{A}}
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item \only<-1>{2D / 2.5D / 3D}\only<2->{{\alert{\underline{2D / 2.5D / 3D}}}}
\item \only<-2>{Векторная / растровая}\only<3->{{\alert{\underline{Векторная / растровая}}}}
\item \only<-3>{Realtime}\only<4->{{\alert{\underline{Realtime}}}} / near real-time / offline
\item \only<-4>{Фотореалистичная / стилизованная}\only<5->{{\alert{\underline{Фотореалистичная / стилизованная}}}}
\item CPU / \only<-5>{GPU}\only<6->{{\alert{\underline{GPU}}}}
\end{itemize}
\end{frame}
...
\begin{itemize}
\pause
\item Как пользоваться графическими API
\pause
...
\pause
\item Профессия \alert{\textit{graphics engineer}}
\end{itemize}
\end{frame}
...
\pause
\item Разработка движков графического интерфейса
\pause
\item И т.д.
\end{itemize}
\end{frame}
...
\pause
\begin{itemize}
\item \only<-11>{OpenGL 3.3 (Khronos Group, 2010)}\only<12->{\alert{\underline{OpenGL 3.3 (Khronos Group, 2010)}}}
\end{itemize}
\pause
...
\pause
\item WebGPU Compute Shaders (W3C, 2021)
\end{itemize}
\end{frame}
...
\begin{itemize}
\item 200 строк кода, чтобы нарисовать один треугольник
\pause
\item Ориентирован на web, но есть реализации под десктоп (\href{https://github.com/gfx-rs/wgpu-native}{\nolinkurl{wgpu-native}})
\pause
\item Средне низкоуровневый API: сложнее (но приятнее), чем OpenGL, и легче, чем Vulkan
\item Хороший выбор для изучения после OpenGL
