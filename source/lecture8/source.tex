...
\pause
\item \begin{math}\Rightarrow\end{math} может работать довольно медленно
\end{itemize}
\end{frame}
...
\begin{itemize}
\item При простом raytracing'е (без решения rendering equation) для получения жёсткой тени можно послать дополнительный луч в направлении света (shadow ray): если он что-то пересёк, точка находится в тени
\pause
...
\pause
\item Процедуру нужно повторить для каждого источника света
\end{itemize}
\end{frame}
...
\pause
\item \begin{math}\Rightarrow\end{math} модель должна быть специально адаптирована для генерации shadow volume
\pause
...
\item Тень -- это то, что \textbf{не видит} источник света
\pause
...
\begin{verbatim}
#version 330 core
// Матрица с которой рисовался shadow map
...
void main(){
  ...
  if (abs(shadow_coord.x) < 1.0 && abs(shadow_coord.y) < 1.0){
    ...
    bool in_shadow = texture(shadow_depth,
      shadow_coord.xy).r < shadow_coord.z;
...
\pause
\item Exponential shadow maps, variance shadow maps (следующая лекция)
\end{itemize}
\end{itemize}
\end{frame}
...
\item \verb|GL_TEXTURE_COMPARE_FUNC| в значение \verb|GL_LEQUAL| (теоретически могут быть \verb|GL_ALWAYS| и т.п.)
\end{itemize}
\pause
\item Текстуру с \verb|GL_COMPARE_REF_TO_TEXTURE| \textbf{нельзя} использовать как \verb|sampler2D|, для неё есть отдельный тип sampler'а: \verb|sampler2DShadow|
\pause
